...
<head>
  ...
  <script src="js files/team-picker.js"></script>
...
<body>
  ...
  <div id="main">
    <h1 style="font-size:35px; text-decoration:underline; color:#fd4503">Random Team Picker</h1>
    <label>Number of Teams: </label>
    <select id="sel" onchange="teams(); showAdvanced();">
      <option disabled selected value value="0"> -- Choose -- </option>
      <option value='2'>2</option>
      <option value='3'>3</option>
      <option value='4'>4</option>
      <option value='5'>5</option>
      <option value='6'>Custom (max 99)</option>
    </select>
    <input type="number" name="custom-teams" id="custom-teams" value="" style="width:35px; display:none;" min="2" max="99" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength); customTeams();" maxlength = "2">
    <div id="team-adv" style="display:none;">
      ...
      <div id="advanced" style="height: 0px;"></div>
    </div>
    <h2 style="margin: 20px 0 10px 10px">Insert each player separated by 'enter' (one each line):</h2>
    <textarea id="rand-input" name="rand-values" height="225px" rows="8" cols="84" style="resize:none; font-size:24px;"></textarea><br>
    <button type="submit" name="submit" style="margin-left:15px; margin-top:20px" onclick="generate()">Generate</button>
    <button type="submit" name="submit" style="margin-left:15px; margin-top:20px" onclick="displayResult()">Update 'More Options'</button>
    <h2 style="margin: 20px 0 10px 10px">Last team distribution:</h2>
    <div class="teams-box output-box" id="output-teams">
    </div>
    <p><b>*Note:</b> Values containing only spaces/ line breaks will not be accepted ! <b>*</b></p>
    <p>By pressing the "Update 'More Options' " button, the values will not be re-shuffled ! Only the colors and the team names will get updated !</p>
    <?php
      $rands[0] = ""; $aux=0;
